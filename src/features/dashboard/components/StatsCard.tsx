    ...
    return (
        <Card className={cn(
            ...
            isPrimary 
                ...
                : "bg-card text-foreground border-border/60 shadow-sm hover:border-slate-300 hover:shadow-md"
        )}>
            {/* Subtle background decoration for primary card */}
            {isPrimary && (
                <div className="absolute -top-20 -right-20 w-48 h-48 bg-card/5 blur-[40px] rounded-full pointer-events-none" />
            )}

            <div className="p-4 md:p-6 relative z-10 flex flex-col h-full justify-between">
                <div>
                    <div className="flex justify-between items-center mb-3 md:mb-6">
                        <p className={cn("text-xs md:text-sm font-black tracking-wide", isPrimary ? "text-primary-foreground/90" : "text-muted-foreground")}>
                            {stat.label}
                        </p>
                        <div className={cn(
                            ...
                            isPrimary ? "bg-card/20 text-white border border-border/10" : cn(stat.iconBg, stat.iconColor, "border border-slate-50")
                        )}>
                    ...
                    <div className="flex items-center gap-3 md:gap-4">
                        ...
                        {!isNeutral && stat.trendValue && (
                            <div className={cn(
                                ...
                                isPrimary 
                                    ? "bg-card/20 text-white border-border/20" 
                                    : isPositiveTrend 
                ...
                <div className={cn(
                    ...
                    isPrimary ? "border-border/10" : "border-border"
                )}>
                    <p className={cn("text-[10px] md:text-xs font-bold w-full truncate", isPrimary ? "text-primary-foreground/80" : "text-muted-foreground")}>
                        {stat.change}
                    </p>
                </div>
            </div>
