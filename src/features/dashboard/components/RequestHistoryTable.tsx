            ...
            <div className="border-b border-slate-100/80 px-4 sm:px-8 py-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-50/30">
                ...
                <div className="relative w-full sm:w-72">
                    ...
                    <Input
                        ...
                        className="ps-10 h-11 text-start bg-white border-slate-200 focus-visible:ring-primary/20 rounded-xl font-medium text-sm transition-all"
                        value={searchTerm}
            ...
            <div className="divide-y divide-slate-100/80">
                {requests.length > 0 ? (
                    requests.map((req) => {
                        ...
                        const status = statusConfig[req.status] || { label: req.status, color: "bg-slate-50 text-slate-600" };
                        ...
                        return (
                            ...
                            >
                                ...
                                <div className="sm:col-span-5 md:col-span-4 flex items-center gap-4">
                                    <div className="w-12 h-12 rounded-[14px] bg-slate-100 flex items-center justify-center shrink-0 border border-slate-200/50 group-hover:border-primary/20 group-hover:bg-primary/5 transition-colors">
                                        <Icon className="w-6 h-6 text-slate-400 group-hover:text-primary transition-colors" />
                                    </div>
                                    <div className="flex-1 min-w-0 space-y-1">
                                        <h4 className="font-black text-slate-900 text-sm truncate group-hover:text-primary transition-colors">{req.title}</h4>
                                        <div className="flex items-center gap-2">
                                            <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{req.id}</span>
                                            <span className="w-1 h-1 rounded-full bg-slate-300" />
                                            <span className="text-xs font-bold text-slate-500">{req.category}</span>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="sm:col-span-4 md:col-span-3 flex flex-row sm:flex-col justify-between sm:justify-center items-start gap-2">
                                    ...
                                    <div className="text-end sm:text-start flex flex-col gap-0.5 mt-1">
                                        <span className="font-black text-slate-900 text-sm">{req.amount}</span>
                                        <span className="text-[10px] text-slate-400 font-bold uppercase tracking-wider">{req.date}</span>
                                    </div>
                                </div>

                                {/* Column 4: Action */}
                                <div className="sm:col-span-3 md:col-span-1 flex justify-end sm:justify-center">
                                    <div className="w-10 h-10 rounded-full bg-white border border-slate-200 flex items-center justify-center group-hover:bg-primary group-hover:border-primary group-hover:text-white transition-all shadow-sm">
